...
def main(unused_argv):
  ...
  reg_flag = False
  reg_fn = train_utils.create_reg_schedule(config)
  ...
  for step in range(init_step, num_steps + 1):
    with jax.profiler.StepTraceAnnotation('train', step_num=step):
      ...
      if config.surface_reg and (step % reg_fn(train_frac) == 0) and not reg_flag:
        reg_flag = True
      else:
        reg_flag = False
        
      state, stats, rngs = train_pstep(rngs, state, batch, cameras, train_frac, loss_threshold, reg_flag,)  # pytype: disable=wrong-arg-types  # jnp-type
