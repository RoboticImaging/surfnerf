...
from internal import permutohedral_grid
...
from internal import gradient_scaling
from internal import surface_reg
import jax
...
class Model(nn.Module):
  ...
  def setup(self):
    ...
    if self.config.surface_sphere_type == "random":
      self.sphere_points = surface_reg.uniform_sphere(PRNG, n = 4096, volume_sample=self.config.surface_vol_samples)
    elif self.config.surface_sphere_type == "vmf":
      self.sphere_points = surface_reg.vMF_sphere_samples(L = self.config.surface_samples, volume_sample=self.config.surface_vol_samples)
    elif self.config.surface_sphere_type == "fib":
      self.sphere_points = surface_reg.fibonacci_sphere(n = self.config.surface_samples, volume_sample=self.config.surface_vol_samples)
    else:
      raise ValueError(
              'Surface sphere type must be one of random, vmf, or fib.'
          )
  ...
  def __call__(
    ...
    if self.config.hash_grid_type == "voxel":
      grid_fn = grid_utils.HashEncoding
    elif self.config.hash_grid_type == "permuto":
      grid_fn = permutohedral_grid.HashEncodingPermuto
    else:
      raise ValueError(
              'hash_grid_type must be either voxel or permuto.'
          )

    grids = [
        grid_fn(name=f'grid_{i}', **params)
    ...
    # Second pass through the network happens independently
    # This way all the memory get's freed without having to grossly refactor things
    if (reg_sample):
      # nerf_level = len(self.sampling_strategy) - 1
      # print(nerf_level)
      nerf_level = -1

      mlp = mlps[nerf_level]
      if (self.sampling_strategy[nerf_level][1] == None):
        grid = None
      else:
        grid = grids[nerf_level]
      gaussians = rays.gaussians
      ...
      if self.scheduled_beta:
        ...
        curr_beta = self.get_scheduled_beta(nerf_level, train_frac)

      key, rng = random_split(rng)
      surface_results = mlp(key,
          gaussians,
          viewdirs=rays.directions,
          imageplane=rays.imageplane,
          glo_vec=jax.lax.stop_gradient(glo_vec),
          exposure=jax.lax.stop_gradient(exposure_values),
          curr_beta=curr_beta,
          grid=grid,
          rays=rays,
          tdist=rays.surface_tdist,
          train=train,)
    ...
    for i_level, (i_mlp, i_grid, num_samples) in enumerate(
      ...
      # Render each ray.
      extras_to_render = ['roughness', 'diffuse', 'specular']
      ...
      if self.config.gradient_scaling:
        ray_results['density'] = gradient_scaling.gradientScaling(rays.origins, gaussians, ray_results['density'])
        if ray_results['rgb'] is not None:
          ray_results['rgb'] = gradient_scaling.gradientScaling(rays.origins, gaussians, ray_results['rgb'])
      ...
      # Implement an additional MLP sampling based on values obtained.
      if self.config.surface_reg:
        weights_mean = jnp.median(weights, axis=-1, keepdims = True)

        surface_approx = (weights > weights_mean).argmax( axis=-1, keepdims=True)
        render_vals = jnp.unravel_index(surface_approx, weights.shape)
        
        # TODO(jack): This will break if surface is at the end of the ray...
        # What is the fix? Something using the size of the array. Hmm. Dodgy.
        tdist_vals = jnp.unravel_index(surface_approx + 1, weights.shape)

        surface_sample_mean = jax.lax.stop_gradient(gaussians[0][render_vals])
        surface_sample_covs = jax.lax.stop_gradient(gaussians[1][render_vals])
        surface_sample_gaussians = (surface_sample_mean, surface_sample_covs)
        tmean = (tdist[...,1:] + tdist[...,:-1])/2
        tdist_mean = tmean[render_vals]


        tdist_0 = tdist[render_vals]
        tdist_1 = tdist[tdist_vals]
        surface_tdist = jnp.concatenate([tdist_0, tdist_1], axis = -1)

        # jax.debug.print("Max dist1-dist0 {x}", x = jnp.max(tdist_1-tdist_0))

        reg_vals['surface_app_samples'] = surface_sample_gaussians
        reg_vals['surface_app_weights'] = weights[render_vals]
        reg_vals['surface_app_pos'] = jax.lax.stop_gradient(tdist_mean) - (weights * tmean).sum(axis = -1, keepdims = True) / jnp.maximum(rendering['acc'], jnp.finfo(jnp.float32).eps)
        reg_vals['surface_tdist'] = surface_tdist
        reg_vals['surface_normals'] = jax.lax.stop_gradient(ray_results['normals'][render_vals])
      
      reg_values.append(reg_vals)
    ...
    return renderings, ray_history, reg_values
...
class MLP(nn.Module):
  ...
  def setup(self):
    ...
    self.downweighting_jacob = coord.contract3_isoscale if not self.isoscale_2d else coord.contract3_isoscale_2d
  ...
  def __call__(
        ...
        if not self.squash_before and self.warp_fn is not None:
          if perp_mag is not None and self.unscented_scale_mult > 0:
            if self.warp_fn.__wrapped__ == coord.contract:
              ...
              s = self.downweighting_jacob(control)
    ...
    if grid is not None:
      ...
      predict_density_kwargs['perp_mag'] = jnp.broadcast_to(perp_mag, control_offsets.shape[:-1])
    ...
    if self.disable_rgb:
      ...
      if viewdirs is not None or glo_vec is not None:
        ...
        if viewdirs is not None:
          ...
          if (viewdirs.ndim == bottleneck.ndim):
            eviewdirs = viewdirs
          else:
            eviewdirs = viewdirs[..., None, :]
          ...
          if self.use_reflections:
            ...
            refdirs = ref_utils.reflect(-eviewdirs, normals_to_use)
        ...
        if self.use_n_dot_v:
          dotprod = jnp.sum(
              normals_to_use * eviewdirs, axis=-1, keepdims=True
          )
          x.append(dotprod)
    ...
    ray_results = dict(
        ...
        roughness=roughness,
        tint = tint if (self.use_specular_tint and not self.disable_rgb) else None,
        diffuse = diffuse_linear if  (self.use_diffuse_color and not self.disable_rgb) else None,
        specular_raw = spec_no_tint if (self.use_diffuse_color and not self.disable_rgb) else None,
        specular = specular_linear if (self.use_diffuse_color and not self.disable_rgb) else None,
    )
...
def render_image(
  ...
  for i_chunk, idx0 in enumerate(idx0s):
    ...
    chunk_renderings, _, _ = render_fn(rng, chunk_rays)
  ...
  if return_all_levels:
    ...
    if 'specular' in rendering:
      rendering['specular'] = rendering['specular'][-1]
      rendering['diffuse'] = rendering['diffuse'][-1]
