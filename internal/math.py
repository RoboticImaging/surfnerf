...
def power_n_log_lerp(n, t, v0, v1, round_func = jnp.round):

  # Log lerp to nearest power of n.
  # Different functions 

  return jnp.power(n, round_func(jnp.log(log_lerp(t, v0, v1))/(jnp.maximum(jnp.log(n),1e-10))))


def cartesian_to_spherical(vals):
  '''
  theta in [0,pi]
  phi in [0, 2*pi) (wraps around)
  r is vec norm
  '''

  r = l2_length(vals)

  theta = jax.lax.acos(vals[...,None,2]/r)

  phi = jax.lax.atan2(vals[...,None,0],vals[...,None,1])

  return jnp.concatenate([theta, phi, r], axis = -1)

def cosine_similarity(a, b):

  return jnp.sum(a * b, axis = -1, keepdims = True)/jnp.maximum(l2_length(a)*l2_length(b), 1e-10)


# Rodrigues Formula to align one vector to another.
def align_vectors(a, b):
  a = l2_normalize(a)
  b = l2_normalize(b)

  c = jnp.sum(a * b, axis=-1, keepdims=True)

  
  I = jnp.eye(3)


  K = jnp.einsum('...i,...j', b, a, precision=jax.lax.Precision.HIGHEST) - jnp.einsum('...i,...j', a, b, precision=jax.lax.Precision.HIGHEST)

  return I + K + jnp.einsum('...ij,...jk->...ik', K, K, precision=jax.lax.Precision.HIGHEST) / jnp.maximum(1 + c[...,None], jnp.finfo(jnp.float32).eps)

# Provide a bunch of random rotation matrixes to rotate our sample during training
def random_rot_matrices(PRNG, matrix, deflection = 1.0):
  #from http://www.realtimerendering.com/resources/GraphicsGems/gemsiii/rand_rotation.c
  shape = (matrix.shape[0],) + (matrix.ndim-2)*(1,)
  keys = jax.random.split(PRNG, 3)
  theta = 2* jnp.pi * deflection * jax.random.uniform(keys[0],(*shape,1),minval=0,maxval=1)
  phi = 2* jnp.pi * deflection * jax.random.uniform(keys[1],(*shape,1),minval=0,maxval=1)
  z = 2 * deflection * jax.random.uniform(keys[2],(*shape,1),minval=0,maxval=1)

  r = jnp.sqrt(z)
  V = jnp.stack([jnp.sin(phi) * r, jnp.cos(phi) * r, jnp.sqrt(2.0 - z)], axis = -1)

  R = jnp.broadcast_to(jnp.eye(3), (*shape,3,3))
  R = R.at[...,0,0].set(jnp.cos(theta)[...,0])
  R = R.at[...,1,1].set(jnp.cos(theta)[...,0])
  R = R.at[...,0,1].set(jnp.sin(theta)[...,0])
  R = R.at[...,1,0].set(-jnp.sin(theta)[...,0])

  M = jnp.einsum('...ij,...jk->...ik', jnp.einsum("...i,...j->...ij", V, V)[...,0,:,:] - jnp.eye(3), R, precision=jax.lax.Precision.HIGHEST)

  return jnp.einsum("...ij,...j->...i", M, matrix)