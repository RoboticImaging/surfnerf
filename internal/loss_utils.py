...
def bilevel_predicted_normal_loss(ray_history, *, coarse_mult=1.0, mult=1.0, predicted_mult=1.0):
  """Computes the predicted normal supervision loss defined in ref-NeRF."""
  total_loss = 0.0
  for i, ray_results in enumerate(ray_history):
    if i < len(ray_history) - 1:
    ...
    loss_normal = jnp.mean((w * (1.0 - jnp.sum(n * jax.lax.stop_gradient(n_pred), axis=-1))).sum(axis=-1))
    loss_pred = jnp.mean((jax.lax.stop_gradient(w) * (1.0 - jnp.sum(jax.lax.stop_gradient(n) * n_pred, axis=-1))).sum(axis=-1))

    total_loss += curr_mult * (loss_normal + predicted_mult * loss_pred)
  return total_loss
...
def surface_spatial_loss(spatial_term, surf_weight, loss_mult):

  total_loss = loss_mult * jnp.mean(surf_weight*jnp.mean(spatial_term, axis=-2))

  return total_loss


def surface_normal_loss(normals_term, surf_weight, loss_mult):

  total_loss = loss_mult * jnp.mean(surf_weight*jnp.mean(normals_term, axis=-2))

  return total_loss


def surface_bias_loss(bias_term, surf_weight, loss_mult):

  total_loss = loss_mult * jnp.mean(surf_weight[...,None]*bias_term)

  return total_loss

def surface_tv_loss(specular_term, surf_weight, loss_mult):
  total_loss = loss_mult * jnp.mean(surf_weight[..., None]*jnp.mean(specular_term, axis=-2))
  return total_loss