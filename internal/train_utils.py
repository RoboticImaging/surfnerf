...
def bilevel_predicted_normal_loss(ray_history, config):
  return loss_utils.bilevel_predicted_normal_loss(
      ray_history,
      coarse_mult=config.predicted_normal_coarse_loss_mult,
      mult=config.predicted_normal_loss_mult,
      pred_mult=config.bilevel_predicted_loss_mult,
  )
...
def surface_spatial_loss(surf_terms, reg_vals, config):
  return loss_utils.surface_spatial_loss(surf_terms['spatial_reg'], 
                                         reg_vals['surface_app_weights'], 
                                         config.surface_spatial_loss_mult,)

def surface_normal_loss(surf_terms, reg_vals, config):
  return loss_utils.surface_normal_loss(surf_terms['normal_reg'], 
                                         reg_vals['surface_app_weights'], 
                                         config.surface_normal_loss_mult,)

def surface_bias_loss(surf_terms, reg_vals, config):
  return loss_utils.surface_bias_loss(surf_terms['bias_reg'], 
                                         reg_vals['surface_app_weights'], 
                                         config.surface_bias_loss_mult,)

def surface_tv_loss(surf_terms, reg_vals, config):
  return loss_utils.surface_tv_loss(surf_terms['specular_reg'], 
                                         reg_vals['surface_app_weights'], 
                                         config.surface_tv_loss_mult,)
...
def create_train_step(
  ...
  if config.surface_reg:
    _key = jax.random.PRNGKey(2206)
    sphere = surface_reg.uniform_sphere(_key, n = 2048) # Can be deterministic - randomly sampled later
  else:
    sphere = None

  def train_step(rng, state, batch, cameras, train_frac, loss_threshold, reg_sample = False):
    ...
    def loss_fn(variables: Any):
      ...
      renderings, ray_history, reg_vals = model.apply(
          variables,
      ...
      losses['data'], stats = compute_data_loss(batch, renderings, rays, loss_threshold, config)
      ...
      if (
        ...
        losses['predicted_normals'] = predicted_normal_loss(ray_history, config) if not config.bilevel_normal_loss else bilevel_predicted_normal_loss(ray_history, config)
      ...
      if config.surface_reg:
        ...
        if reg_sample:
          ...
          nerf_sample = reg_vals[-1]


          surface_reg_rays = surface_reg.surface_reg_batch(rng, sphere, nerf_sample['surface_app_samples'], rays, jax.lax.stop_gradient(nerf_sample['surface_tdist']), nerf_sample['surface_normals'], surface_dir_reg_front=config.surface_visible_dirs, sample_points = config.surface_samples)


          surface_results = model.apply(
              variables,
              key if config.randomized else None,
              surface_reg_rays,
              train_frac=train_frac,
          ...
          surf_reg_losses = surface_reg.surface_reg_losses(surface_results, surface_reg_rays.gaussians[0], surface_reg_rays.directions, config)

          

          if (config.surface_spatial_loss_mult > 0.0):

            spatial_loss = surface_spatial_loss(surf_reg_losses, nerf_sample, config)

          if (config.surface_normal_loss_mult > 0.0):

            normals_loss = surface_normal_loss(surf_reg_losses, nerf_sample, config)

          if (config.surface_bias_loss_mult > 0.0):

            bias_loss= surface_bias_loss(surf_reg_losses, nerf_sample, config)

          if (config.surface_tv_loss_mult > 0.0):

            tv_loss = surface_tv_loss(surf_reg_losses, nerf_sample, config)

        losses['spatial_reg'] = spatial_loss
        losses['normals_reg'] = normals_loss
        losses['bias_reg'] = bias_loss
        losses['specular_reg'] = tv_loss
  ...
  if config.surface_reg:
    ...
    train_pstep = jax.pmap(
        ...
        donate_argnums=(0, 1),static_broadcasted_argnums=(6,))
  else:
    train_pstep = jax.pmap(
        train_step,
        axis_name='batch',
...
def reg_schedule(train_frac, freq_init, freq_final):
  schedule = lambda f: math.power_n_log_lerp(2, f, freq_init, freq_final)
  return schedule(train_frac) # If the step is a multiple of the current reg frequency, we should regularise.

def create_reg_schedule(config):

  def get_reg_fn(freq_init, freq_final):
    return functools.partial(reg_schedule,
                             freq_init=freq_init,
                             freq_final=freq_final)
  
  reg_fn_main = get_reg_fn(config.surface_reg_sched_init, config.surface_reg_sched_final)
  return reg_fn_main
