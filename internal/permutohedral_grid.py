...
from internal import permutohedral_tools
import jax
...
def permuto_interp(
  ...
  if datastructure == 'hash':
    fn = permutohedral_tools.hash_resample_permuto
    ...
    def fn(v, c):
      ...
      return permutohedral_tools.resample_permuto(v, c[None, None], grid_size)[0, 0]
...
@gin.configurable
class HashEncodingPermuto(nn.Module):  # TODO(barron): Rename this to just "NGP".
  ...
  def __call__(
    ...
    for grid_size in self.grid_sizes:
      ...
      # We are now operating in a 4D space, so, the grid must now be grid^4
      # BUT WAIT - there's a hack, all coords add to 0, so we only need 3 to uniquely define.
      if (8*grid_size)**3 <= self.hash_map_size:
        # For smaller levels (fewer cells), store entries in a dense grid.
        datastructure = 'grid'
        shape_prefix = [8*grid_size] * 3
      else:
        datastructure = 'hash'
    ...
    for values, grid_size, datastructure in zip(
      ...
      f = permuto_interp(values, x_scaled, datastructure, grid_size)
      ...
      if x_scale is not None:
        ...
        # For a permutohedral grid, it is *apparently* (well, I couldn't do it anyway) difficult 
        # to integrate a 3D gaussian over a tetrahedron. Instead, I have integrated over a sphere
        # and then multiplied this by the added variance from slicing/splatting in a permutohedral
        # lattice. The sphere is of radius sqrt(3) / 4n in this case, given the canonical simplex.
        weighting = 4*jnp.sqrt(1/6) * (- math.safe_exp(-3/(32*x_scale**2 * grid_size**2)) * jnp.sqrt(6/(jnp.pi * x_scale**2)) / (4 * grid_size) + math.approx_erf(jnp.sqrt(3/2)/(4*grid_size * x_scale)))
        f *= weighting
