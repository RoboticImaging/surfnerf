...
# From NeRF-Casting
def contract3_isoscale_2d(x):
  if x.shape[-1] != 3:
    raise ValueError(f'Inputs must be 3D, are {x.shape[-1]}D.')
  norm_sq = jnp.maximum(1, jnp.sum(x**2, axis=-1))

  return jnp.exp(2 * (jnp.log(2 * jnp.sqrt(norm_sq) - 1) - (2 * jnp.log(norm_sq))))
...
def hexify(rng, *, origins, directions, radii, tdist):
  ...
  if origins.ndim == 5:
    control = (
      ...
      + perp_axis1[Ellipsis, None, :] * cx[Ellipsis, None]
      + perp_axis2[Ellipsis, None, :] * cy[Ellipsis, None]
      + directions[Ellipsis, None, :] * cz[Ellipsis, None]
  )
  else:
    control = (
