          ...
          if config.compute_disp_metrics:
            for tag in ['mean', 'median']:
              ...
              if key in rendering:
                ...
                metric[f'disparity_{tag}_rmse'] = float(
                    np.sqrt(((disparity - batch.disps) ** 2).mean())
                )

                metric[f'disparity_{tag}_absrel'] = float(
                    (np.abs(batch.disps - disparity) / (1 + batch.disps)).mean()
                )

                metric[f'disparity_{tag}_sqrel'] = float(
                    (np.abs(batch.disps - disparity)**2 / ((1+batch.disps) ** 2)).mean()
                )
          ...
          if (idx % config.eval_render_interval) == 0:
            ...
            for key in ['distance_mean', 'distance_median']:
              if key in rendering:
                ...
                if config.eval_save_colour_depth:
                  distance_limits = config.near, config.far
                  lo, hi = [render_dist_curve_fn(x) for x in distance_limits]


                  img = np.clip((render_dist_curve_fn(rendering[key][-1]) - lo) / (hi - lo), 0, 1)
                  # Flip directions so that red=close and blue=far.
                  img = colormaps.get_cmap('turbo')(1 - img)[Ellipsis, :3]


                  image_io.save_img_u8(
                  img,
                  path_fn(f'{key}_rgb_{idx:03d}.png'),
                  )
            ...
            if 'specular' in rendering:
              image_io.save_img_u8(
                rendering['specular'],
                path_fn(f'color_specular_{idx:03d}.png'),
              )
            
            if 'diffuse' in rendering:
              image_io.save_img_u8(
                rendering['diffuse'],
                path_fn(f'color_diffuse_{idx:03d}.png'),
              )
